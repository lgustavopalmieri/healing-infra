...
#
# Since module.eks.private_subnet_ids is unknown at plan time on first apply,
# we cannot use for_each over those IDs. Instead we use the statically-known
# AZ list to compute which indices are supported, then select the matching
# subnet IDs by index — this keeps the filtering plan-time safe.
###############################################################################
